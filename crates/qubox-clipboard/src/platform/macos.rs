//! macOS clipboard backend via `arboard`.
//!
//! `arboard` on macOS uses NSPasteboard. Like the other platform
//! backends, we construct a fresh `Clipboard` per call to satisfy
//! the `!Send + !Sync` constraint.
