//! Linux clipboard backend via `arboard`.
//!
//! arboard on Linux uses `wlr-data-control` on Wayland and
//! `x11-clipboard` on X11 transparently. We construct a fresh
//! `Clipboard` per call to respect the `!Send + !Sync` constraint.

use qubox_proto::ClipboardPayload;

use super::{ClipboardError, ClipboardImage, ClipboardSnapshot};

/// Maximum size of a clipboard image we will encode, in bytes of
/// RGBA pixels (width * height * 4). Above this the snapshotter
/// returns `None` for the image rather than allocating megabytes.
            ...
            Ok(image) => {
                if !image.bytes.is_empty() {
                    ...
                    if pixels <= MAX_IMAGE_PIXELS as u64 {
                        ...
                        tracing::warn!(
                            pixels,
                            "clipboard image too large; skipping this iteration"
                        );
                    }
                }
            }
